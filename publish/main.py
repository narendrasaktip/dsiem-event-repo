...
# =========================================================
# Fungsi dan Pemuatan Konfigurasi Hybrid
# =========================================================
def load_master_config(path="config.json"):
    """Membaca file konfigurasi utama (config.json)."""
    try:
        with open(path, 'r') as f:
            return json.load(f)
    except (IOError, json.JSONDecodeError) as e:
        raise SystemExit("[FATAL] Gagal membaca atau memproses file '{}': {}. Pastikan file ada dan formatnya benar.".format(path, e))

# --- Memuat Konfigurasi dari Awal ---
print("[INFO] Membaca konfigurasi terpusat dari config.json...")
cfg = load_master_config()

# --- Variabel dari config.json (Non-Rahasia) ---
GITHUB_REPO = cfg.get("github", {}).get("repo")
GITHUB_BRANCH = cfg.get("github", {}).get("branch")
ES_HOST = cfg.get("opensearch", {}).get("host")
# Anda bisa menambahkan variabel lain dari config.json di sini jika perlu

# --- Variabel dari Environment (Rahasia) ---
GITHUB_TOKEN = os.getenv("GITHUB_TOKEN")
ES_PASSWD_FILE = os.getenv("ES_PASSWD_FILE")
ES_USER_LOOKUP = os.getenv("ES_USER_LOOKUP")
EMAIL_APP_PASSWORD = os.getenv("EMAIL_APP_PASSWORD") # Meskipun tidak dipakai di skrip ini, ini pola yang benar

# Variabel lain yang bisa tetap di environment
...
DICT_REFRESH_INTERVAL = int(os.getenv("DICT_REFRESH_INTERVAL", "60"))
...
def collect_field_mappings(cfg, use_remote_defaults, directive_category=None, full_slug=None):
    ...
    # <-- TAMBAHAN: Logika untuk field yang diisi otomatis penuh -->
    pre_filled_mappings = {}
    if directive_category:
        pre_filled_mappings["category"] = {"mode": "h", "value": directive_category}
    if full_slug:
        pre_filled_mappings["subcategory"] = {"mode": "h", "value": full_slug}
    # <-- AKHIR TAMBAHAN -->

    # Definisikan default spesial (ketika input dikosongkan)
    special_defaults = {
        "sensor": ("f", "syslog_hostname"),
        "product": ("f", "device_product"),
        "protocol": ("h", "TCP")
    }

    def get_mapping(key, default_field=None, special_default=None):
        ...
        default_mode_ask = 'f'
        if special_default:
            default_mode_ask = special_default[0]

        mode = ask_hf(key, default_mode=(m_default if m_default in ("h","f") else default_mode_ask))
        
        while True:
            default_text = ""
            if special_default:
                default_text = " (kosongkan untuk default ke '{}')".format(special_default[1])
            elif mode=="f" and default_field:
                default_text = " (default: {})".format(default_field)
            
            val = py_input("{} value{}: ".format(key, default_text)).strip()

            if not val and special_default:
                print("  -> Menggunakan default otomatis: Mode='{}', Value='{}'".format(special_default[0].upper(), special_default[1]))
                return special_default[0], special_default[1]
    ...
    for key, default in fields_to_map:
        # <-- UBAH LOGIKA LOOP INI -->
        if key in pre_filled_mappings:
            mappings[key] = pre_filled_mappings[key]
            print("[INFO] Otomatis mengisi '{}': Mode='{}', Value='{}'".format(key, mappings[key]['mode'].upper(), mappings[key]['value']))
            continue # Lanjut ke field berikutnya
        
        special_default_tuple = special_defaults.get(key)
        mode, value = get_mapping(key, default_field=default, special_default=special_default_tuple)
        # <-- AKHIR PERUBAHAN -->
    ...
    if not ts_in:
        ...
        ts_in = py_input("Field timestamp (default: syslog_timestamp): ").strip() or "syslog_timestamp"
    else:
...
def get_rule_template(template_name="default", plugin_id=None, sid=None, title=""):
    templates = {
        "default": [
            ...
            {"stage": 2, "name": title, "plugin_id": plugin_id, "plugin_sid": [sid], "occurrence": 10, "reliability": 5, "timeout": 3600, "from": ":1", "to": "ANY", "port_from": "ANY", "port_to": "ANY", "protocol": "ANY", "type": "PluginRule", "custom_data1": "ANY", "custom_data2": "ANY", "custom_data3": "ANY"},
...
# ====== MAIN ======
def main():
    # Validasi bahwa semua konfigurasi penting sudah termuat
    require_github()
    ...
    device_name = py_input("\nNama device/log source (untuk folder & index, cth: fortigate): ").strip()
    log_type_auto = sanitize(device_name)
    index_pattern = "{}-*".format(log_type_auto)
    print("[INFO] Index pattern diatur otomatis ke: {}".format(index_pattern))
    ...
    added = False # Definisikan 'added' di luar blok if
    if existing_file is None:
        rows_for_local = rows[:]
        added = True # Anggap sebagai penambahan karena file baru
        ...
        merged_rows, added_list = merge_dictionary(existing_rows, new_events)
        rows_for_local = merged_rows
        if not added_list:
            print("[SYNC] Up-to-date. Tidak ada penambahan.")
        else:
            added = True # Set flag jika ada penambahan
            print("[SYNC] Tambahan baru: {}".format(len(added_list)))
