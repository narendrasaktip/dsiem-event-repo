...
def main():
    # --- Blok BARU untuk manajemen nama customer dari config.json ---
    config_file = "config.json"
    current_name = ""
    needs_update = False

    # 1. Cek file config.json
    if os.path.exists(config_file):
        try:
            ...
            print("[WARN] Gagal membaca file {}. Anda akan diminta untuk mengaturnya.".format(config_file))
            needs_update = True
    else:
    ...
    if needs_update:
        ...
        # Simpan ke dalam struktur config.json
        # Baca dulu config yang ada jika ada, untuk mempertahankan setting lain
        full_config = {}
        if os.path.exists(config_file):
            with open(config_file, 'r') as f:
        ...
        if "customer_info" not in full_config:
            full_config["customer_info"] = {}
        full_config["customer_info"]["customer_name"] = new_name
        
        save_json_utf8(config_file, full_config)
        print("[OK] Nama customer diatur di '{}'".format(config_file))
        print("="*50)
    else:
